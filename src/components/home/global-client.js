  ...
  return (
    <>
      ...
      <section className="px-[1.5rem] py-[2.5rem] relative w-full flex flex-col gap-[1rem] justify-center items-center bg-[#0A142F]">
        ...
        <div className="flex flex-wrap gap-[0.5rem] max-w-[90rem] w-full">
          {metServices.map((cs) => {
            const cover = cs?.fields.bannerImage;
            console.log("cover", cover);
            console.log("cover?.fields?.file?.url", cover?.fields?.file?.url);
            return (
              <>
                <div
                  key={cs?.sys?.id}
                ...
                >
                  <Image
                    ...
                    width={400}
                    height={250}
                    className="rounded-[1rem] h-[26rem] w-full object-[center] object-cover"
                  />
                  <h5 className="text-[#F8FAFC] text-[1.25rem] font-[600] mt-[1rem] mb-[0.5rem] absolute top-0 left-[1rem] text-[1.1rem]">
                    <Link
                      ...
                      className="font-[600] text-[#F8FAFC]"
                    >
                      {cs.fields.title}
                    </Link>
                  </h5>
                  <p className="text-[#F8FAFC] text-[0.95rem] font-[400] mt-[1rem] mb-[0.5rem] absolute bottom-0 left-[1rem] group-hover:opacity-100 opacity-0 transition-all duration-300">
